...
- AC-6: closed by **ITER-0005b-FIX-B** (Path 6A — orchestrator `recover_and_rebind` shipped in `fmpl-core/src/lib.rs`; reuses existing `recover_incompatible` closure seam, no new trait. AC text amended per T3 option (b): "logs the recovery attempt" → "the recovery attempt is reflected in `RecoveryStats::recovered_from_source`". Bind-and-execute journey evidence at `fmpl-persistence/tests/scenario_0102_recover_incompatible.rs`). Cardinality stress evidence added by **ITER-0005b-FIX-B-GAP-1** (2026-05-16): SCENARIO-0102 promoted to sentinel cadence; new SCENARIO-0102-multi-stress (N=10 pure incompatible) and SCENARIO-0102-multi-mixed (K=5 compatible + N=10 incompatible) registered at sentinel cadence. Both confirm fjall's `iter()` snapshot-isolates over in-iteration `insert` mutations — the R-B-S-1 concern is resolved by the iterator's existing semantics, no code change to `recover_incompatible` was needed. Error-path symmetric evidence added by **ITER-0005b-FIX-B-GAP-2** (2026-05-16): new test `recover_and_rebind_counts_non_utf8_source_as_recompile_failure` proves non-UTF-8 source bytes route through `RecoveryError::recompile` and increment `recompile_failed` (mirror of the existing non-UTF-8 KEY test). Test passed TDD-green; the path was already correctly handled at `fmpl-core/src/lib.rs:259-260`, no code change needed.
