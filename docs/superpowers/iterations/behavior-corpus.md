...
| SCENARIO-0102 | Loader recovers from incompatible payload via source recompilation | integration | sentinel | `cargo test -p fmpl-persistence --features fjall-backend --test scenario_0102_recover_incompatible` | STORY-0100 |
| SCENARIO-0102-multi-stress | `recover_and_rebind` correctly counts and rebinds at N=10 pure-incompatible cardinality — proves iterator-during-mutation safety under fjall snapshot semantics | integration | sentinel | `cargo test -p fmpl-persistence --features fjall-backend --test recover_and_rebind_unit recover_and_rebind_multi_incompatible_stress` | STORY-0100 |
| SCENARIO-0102-multi-mixed | `recover_and_rebind` correctly handles mixed-cardinality (K=5 compatible + N=10 incompatible) in one keyspace — proves `loaded_passthrough` and `recovered_from_source` increment independently under in-iteration mutation | integration | sentinel | `cargo test -p fmpl-persistence --features fjall-backend --test recover_and_rebind_unit recover_and_rebind_multi_mixed_cardinality` | STORY-0100 |
