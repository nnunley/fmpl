...
- [x] **`specs/pattern-matching.md`**: This spec claims that map and list patterns are not supported in `@` match expressions, but this seems to be a key feature of the "unified grammar" vision. This needs to be clarified and the spec updated to reflect the current implementation or the intended design. ✓ Fixed: Added explicit disclaimers to all examples showing map and list patterns in @ match expressions, clarifying that these features are planned but not yet implemented. Provided let-destructuring workarounds for each example.
